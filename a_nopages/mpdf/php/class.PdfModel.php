...
class rmDir extends img
{
    ...
    {
        $result = parent::getFileNames(); //폴더 리턴임 파일x *주의* 경로 접미사는 없음
        $fileDate = self::date();

        for($i = 0; $i < count($result); $i++) {
            if(substr($result[$i],1,4) != $fileDate) {

                //삭제코드 작성
                $FName = $this->path.$result[$i]."/".$result[$i]."-1.jpg";
                $confFile = date("md",strtotime("md","-5day"));
                if(substr($FName,5,4) >= $confFile) continue;

                @unlink($FName);

                $Fdir = $this->path.$result[$i];

                rmdir($Fdir);

                $this->deleteCnt++; //나중에 쓸데있을까봐 카운트 해놈..
            }

            //substr($result[$i]);
            //print substr($result[$i],1,4);
            //print "<br>";
        }
