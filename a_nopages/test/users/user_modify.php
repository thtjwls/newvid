...
<?
	$sql	="select name, id, nick, email, address, hp from users where no='$userno'";
	$result	=mysql_query($sql,$connect);
	$name	=mysql_result($result,0,0);
	$id		=mysql_result($result,0,1);
	$nick	=mysql_result($result,0,2);
	$email	=mysql_result($result,0,3);
	$address=mysql_result($result,0,4);
	$hp		=mysql_result($result,0,5);
...
?>
	...
	-->
	<form name="user_modify_form" id="user_add" action="user_modify_insert.php" method="post">
	<table cellspacing="0" cellpadding="0" id="user_add_table">
		<tr>
			...
			<td>
				<div>
					<p class="modify" style="color : black; font-size:14px;"><?=$name?></p>
				</div>
			</td>
		</tr>
		<tr>
			<th scope="row" scope="row" id="th_id" style="padding-top : 20px;">
				* 아이디
			</th>
			<td>
				<div class="add_confirm" id="add_confirm">
					<p class="modify" style="color : black; font-size:14px;"><?=$id?></p>
				</div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<div>
					<p class="modify" style="color : black; font-size:14px;"><?=$nick?></p>
				</div>
			</td>
		</tr>
		<tr>
			...
			<td>
				<div>
					<input type="text" name="email1" id="email1" class="user_add_email" value="<?=$email1?>">@
					<input type="text" name="email2" id="email2" class="user_add_email" value="<?=$email2?>">
				</div>
			</td>
		</tr>
		<tr>
			...
			<td>
				<div>
					<input type="text" name="address" id="address" class="user_add_address" value="<?=$address?>">
				</div>
			</td>
		</tr>
		<tr>
	...
	<div id="add_button">
	<input type="button" id="button" value="확인" onclick="user_modify_check()"><input type="reset" id="user_reset" value="취소">
	</div>
	</form>
</div>
</div><!-- wap_end -->
