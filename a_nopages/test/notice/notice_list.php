...
<div id="wap">
	...
	?>
			<tr>
				...
				if($search==""){
					$sql="select * from board where cate='notice' order by idx desc"; 
					...
					$query="select count(*) from board where cate='notice'"; //notice_list 레코드 계산
					...
					$sql="select * from board where $find like '%$search%' and cate='notice' order by idx desc";
					...
					$query="select count(*) from board where $find like '%$search%' and cate='notice'"; //notice_list 레코드 계산
					$result_query=mysql_query($query,$connect);
					$total_record=mysql_result($result_query,0,0);
			...
			<td id="subject"><a href="http://thtjwls.dothome.co.kr/test/notice/notice_view.php?idx=<?=$idx?>"><?=$subject?> <?
		if($total_reple > 0)
		{
			echo "[".$total_reple."]";
		}
...
<div class="paging">
	...
	//페이징 출력
		...
		echo ("<tr id='paging'><td colspan=5><a href='http://thtjwls.dothome.co.kr/test/notice/notice_list.php?page=$before_page'><span>◀ 이전</span></a>");
		}
		...
		for($i=$start_page; $i<=$end_page; $i++){
			if($page == $i){
				...
				echo("<a href='http://thtjwls.dothome.co.kr/test/notice/notice_list.php?page=$i'><span>$i</span></a>");
			}
		}
		if($page < $total_page){
				echo("<a href='http://thtjwls.dothome.co.kr/test/notice/notice_list.php?page=$end_page'><span>다음 ▶</span></a></tr>");
		}else{
	...
	<form id="search_form" action="notice_list.php" method="post">
		...
		<a href="#notice_write" onclick="notice_check()" id="board_write" style="float : right;"><img src="http://thtjwls.dothome.co.kr/test/img/write.png"></a>		
	</form>
