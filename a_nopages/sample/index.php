...
<head>
    ...
    */
	function useradd_go(){
		location.href="user/user_add_form.php";
	}
...
<body>
    <div id="login_wap">
        <div id="login_text">
            <h1>회원 로그인</h1>
            <p>
                회원님의 아이디와 비밀번호를 입력하신 후,<br>
                로그인 버튼을 클릭해 주세요.
            </p>
        </div>
        <div id="login_div">
            <form name="login_form" id="login_form" method="post" action="user_add_check.php?mod=login">
                <div id="input">
                    ...
                    <script>
                             ...
                             var $pass= $('#pass').val();
                             if($id == ""){
                                $('.login_confirm').html("아이디를 입력 해주세요.");                                  
                                } else if ($pass =="") {
                                $('.login_confirm').html("비밀번호를 입력 해주세요.");   
                                } else {
                                    $.ajax({
                                        url:'user/user_add_check.php?mod=login',
                                        type:'post',
                                        data:$('form').serialize(),
                                        success:function(data){
                                            $('.login_confirm').html(data);
                                        }
                                    })
                                }
                            })
